...
type UserName = Omit<UserData, 'name'| 'apartments'> // <--------- создайте тип, состоящий из интерфейса UserData, со всеми исключенными полями, кроме name
...
function mergeObjects<T, U>(obj1: T, obj2: U): T & U {
  ...
  const personData = { city: 'New York', job: 'Developer' };
  
  const mergedObj = mergeObjects(person, personData);
